...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_llm_conversations_tenant_id", "llm_conversations", ["tenant_id"]
    )
    op.create_index("ix_llm_conversations_user_id", "llm_conversations", ["user_id"])
    op.create_index(
        "ix_llm_conversations_context_type", "llm_conversations", ["context_type"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["conversation_id"], ["llm_conversations.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_llm_messages_conversation_id", "llm_messages", ["conversation_id"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["message_id"], ["llm_messages.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(["user_id"], ["users.id"], ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_llm_feedbacks_conversation_id", "llm_feedbacks", ["conversation_id"]
    )
